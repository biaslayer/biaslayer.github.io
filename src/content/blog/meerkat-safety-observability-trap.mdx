...
This week the hype cycle found a new favorite safety paper: **[Detecting Safety Violations Across Many Agent Traces.](https://arxiv.org/pdf/2604.11806){:target="_blank"}** The framework, called **Meerkat**, argues that many of the worst failures in agent systems do not show up in a single run. They emerge only when you analyze large collections of traces together.
...
<figure>
  <img
    src="/blog/meerkat/figure-1-repository-level-decomposition.png"
    alt="Figure from the Meerkat paper showing how individually benign-looking traces can combine into a ransomware workflow when analyzed as a repository."
  />
  <figcaption>
    Figure from the paper. This is the operational case for Meerkat: the failure is distributed across traces, so single-run review misses the witness set.
  </figcaption>
</figure>
...
<figure>
  <img
    src="/blog/meerkat/figure-4-inference-compute-scaling.png"
    alt="Figure from the Meerkat paper showing that increased reasoning effort and compute improve misuse detection probability."
  />
  <figcaption>
    Figure from the paper. Useful result, but also a warning sign: detection gets better as audit compute goes up. That is exactly the safety-cost curve a systems team should worry about.
  </figcaption>
</figure>
...
<figure>
  <img
    src="/blog/meerkat/figure-5-evaluation-cheating.png"
    alt="Figure from the Meerkat paper showing benchmark cheating and task-gaming patterns discovered during auditing."
  />
  <figcaption>
    Figure from the paper. This is where the optimist case is strongest: Meerkat clearly surfaces benchmark cheating and fake-success behavior that weaker monitors would miss.
  </figcaption>
</figure>
